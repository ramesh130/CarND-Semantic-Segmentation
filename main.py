...
def conv_1x1(input, num_classes):
    convolved = tf.layers.conv2d(input, num_classes,
                                 1,
                                 padding='same',
...
def upsample(input, num_classes, kernel_size, strides):
    upsampled = tf.layers.conv2d_transpose(input, num_classes, kernel_size=kernel_size, strides=strides,
                                           padding='same', kernel_initializer=initializer)

    return upsampled


def skip_layer(source, destination):
    return tf.add(source, destination)
...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    model = upsample(layer3_skip, num_classes, 16, (8, 8))
    return model
